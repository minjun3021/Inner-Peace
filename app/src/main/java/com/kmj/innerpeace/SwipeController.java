...
public class SwipeController extends Callback {
    ...
    private static final float buttonWidth = 400;  // 스와이프 했을 때 차지하는 길이
    ...
    private void drawButtons(Canvas c, RecyclerView.ViewHolder viewHolder) {
        ...
        //RectF leftButton = new RectF(itemView.getRight(), itemView.getTop());
        ...
        p.setColor(Color.parseColor("#8d66b3"));

        c.drawRoundRect(right2Button, corners, corners, p);
        drawText("듣기", c, right2Button, p);

        RectF rightButton = new RectF(itemView.getRight() - buttonWidthWithoutPadding, itemView.getTop(), itemView.getRight(), itemView.getBottom());
        p.setColor(Color.parseColor("#ffffff"));
        c.drawRoundRect(rightButton, corners, corners, p);
        drawText2("profile", c, rightButton, p);
    ...
    private void drawText2(String text, Canvas c, RectF button, Paint p) {
        float textSize = 45;
        p.setColor(Color.parseColor("#121319"));
        p.setAntiAlias(true);
        p.setTextSize(textSize);
